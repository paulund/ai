...
- **N child slice issues** — each one shippable by `/ship` in a single PR, labelled `ready,afk,p{1,2,3}`.
- **M marketing issues** — blog posts, launch assets, social content, labelled `ready,hitl,marketing`.
...
gh issue create \
  ...
  --label "ready,afk,p1,feature,<project-scope-label-if-any>" \
...
gh issue create \
  ...
  --label "ready,hitl,marketing,<project-scope-label-if-any>" \
