...
After grilling, update the issue: apply the execution label `afk` (or `hitl` if human judgment is still needed), and append the grilling output to the body.
...
- In step 4, label every issue with exactly one category (`chore`) and exactly one state (`planned`), per the `ops-triage` scheme. After grilling (Step 3), also apply exactly one execution label (`afk` or `hitl`).
